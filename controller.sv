// ===============================================================
// controller.sv
// Controlador SIMD con RAM síncrona (1 ciclo de latencia)
// FSM corregida con estados separados de ADDR/DATA para LOAD_TOP y LOAD_BOTTOM
// ===============================================================

`timescale 1ns/1ps
import interp_pkg::*;

module controller #(
    parameter int LANES      = 4,
    parameter int FRAC       = FRAC_BITS,
    parameter int IMG_W      = 8,     // para depuración rápida (luego 512)
    parameter int IMG_H      = 8,
    parameter int ADDR_W     = 19
)(
    input  logic              clk,
    input  logic              rst_n,

    // Señal de stepping funcional: cuando es 1 se permite avanzar un ciclo
    input  logic              allow_tick,

    input  logic              start,
    output logic              done,

    // RAM de entrada (solo lectura)
    output logic              in_we,
    output logic [ADDR_W-1:0] in_addr,
    output logic [7:0]        in_wdata,
    input  logic [7:0]        in_rdata,

    // RAM de salida (solo escritura)
    output logic              out_we,
    output logic [ADDR_W-1:0] out_addr,
    output logic [7:0]        out_wdata
);

    // ===============================================================
    // 1. Estados y contadores
    // ===============================================================

    typedef enum logic [3:0] {
        S_IDLE,
        S_LOAD_TOP_ADDR,
        S_LOAD_TOP_DATA,
        S_LOAD_BOTTOM_ADDR,
        S_LOAD_BOTTOM_DATA,
        S_COMPUTE,
        S_WRITE,
        S_NEXT,
        S_DONE
    } state_t;

    state_t state, next_state;

    localparam int BLOCKS_X = IMG_W / LANES;

    logic [$clog2(BLOCKS_X)-1:0] x_block;     // bloque horizontal
    logic [$clog2(IMG_H)-1:0]    y_block;     // fila de salida

    // 0..(2*LANES-1): para leer p00/p10 o p01/p11
    logic [$clog2(2*LANES)-1:0]  load_cnt;

    // 0..(LANES-1): para escribir pixel_out_vec
    logic [$clog2(LANES)-1:0]    lane_wr_idx;

    // ===============================================================
    // 2. Vectores de vecinos y salidas SIMD
    // ===============================================================
    logic [7:0] p00_vec[LANES];
    logic [7:0] p10_vec[LANES];
    logic [7:0] p01_vec[LANES];
    logic [7:0] p11_vec[LANES];

    logic [7:0] pixel_out_vec[LANES];

    // ===============================================================
    // 3. Instancia de SIMD4 (4 núcleos secuenciales en paralelo)
    // ===============================================================

    // Por ahora: fx = fy = 0.0
    localparam logic [FRAC-1:0] FX_CONST = '0;
    localparam logic [FRAC-1:0] FY_CONST = '0;

    // Entradas vectoriales constantes para todos los lanes
    logic [FRAC-1:0] fx_vec[LANES];
    logic [FRAC-1:0] fy_vec[LANES];

    genvar gi;
    generate
        for (gi = 0; gi < LANES; gi = gi + 1) begin : GEN_FX_FY
            assign fx_vec[gi] = FX_CONST;
            assign fy_vec[gi] = FY_CONST;
        end
    endgenerate

    logic              simd_valid_in;
    logic              simd_valid_out;

    interp_simd4 #(
        .LANES(LANES),
        .FRAC(FRAC)
    ) u_simd4 (
        .clk        (clk),
        .rst_n      (rst_n),
        .valid_in   (simd_valid_in),
        .p00_vec    (p00_vec),
        .p10_vec    (p10_vec),
        .p01_vec    (p01_vec),
        .p11_vec    (p11_vec),
        .fx_vec     (fx_vec),
        .fy_vec     (fy_vec),
        .valid_out  (simd_valid_out),
        .pixel_out_vec(pixel_out_vec)
    );

    // ===============================================================
    // 4. Cálculo de direcciones de vecinos
    // ===============================================================
    logic [ADDR_W-1:0] addr_p00_vec[LANES];
    logic [ADDR_W-1:0] addr_p10_vec[LANES];
    logic [ADDR_W-1:0] addr_p01_vec[LANES];
    logic [ADDR_W-1:0] addr_p11_vec[LANES];

    integer k;

    always_comb begin
        for (k = 0; k < LANES; k = k + 1) begin
            logic [ADDR_W-1:0] x_pix;
            logic [ADDR_W-1:0] y_pix;
            logic [ADDR_W-1:0] x0, x1, y0, y1;

            // Coordenadas de salida del lane k
            x_pix = x_block*LANES + k;
            y_pix = y_block;

            x0 = x_pix;
            x1 = x_pix + 1;
            y0 = y_pix;
            y1 = y_pix + 1;

            addr_p00_vec[k] = y0*IMG_W + x0;
            addr_p10_vec[k] = y0*IMG_W + x1;
            addr_p01_vec[k] = y1*IMG_W + x0;
            addr_p11_vec[k] = y1*IMG_W + x1;
        end
    end

    // ===============================================================
    // 5. Lógica de salida hacia RAM
    // ===============================================================

    assign in_we   = 1'b0;
    assign in_wdata = 8'd0;

    // Por defecto: no escribir
    assign out_we   = (state == S_WRITE);

    // Dirección de salida: misma que p00 de cada lane
    always_comb begin
        out_addr   = '0;
        out_wdata  = '0;
        if (state == S_WRITE) begin
            out_addr  = y_block*IMG_W + (x_block*LANES + lane_wr_idx);
            out_wdata = pixel_out_vec[lane_wr_idx];
        end
    end

    // ===============================================================
    // 6. Máquina de estados: siguiente estado
    // ===============================================================

    always_comb begin
        next_state   = state;
        simd_valid_in = 1'b0;
        done         = 1'b0;
        in_addr      = '0;

        case (state)
            S_IDLE: begin
                if (start) begin
                    next_state = S_LOAD_TOP_ADDR;
                end
            end

            // ------------------------
            // LOAD TOP (ADDR/DATA)
            // ------------------------
            S_LOAD_TOP_ADDR: begin
                // Primeros LANES accesos: p00, siguientes LANES: p10
                if (load_cnt < LANES)
                    in_addr = addr_p00_vec[load_cnt];
                else
                    in_addr = addr_p10_vec[load_cnt-LANES];

                next_state = S_LOAD_TOP_DATA;
            end

            S_LOAD_TOP_DATA: begin
                // aquí se captura in_rdata (en always_ff)
                if (load_cnt == 2*LANES-1)
                    next_state = S_LOAD_BOTTOM_ADDR;
                else
                    next_state = S_LOAD_TOP_ADDR;
            end

            // ------------------------
            // LOAD BOTTOM (ADDR/DATA)
            // ------------------------
            S_LOAD_BOTTOM_ADDR: begin
                if (load_cnt < LANES)
                    in_addr = addr_p01_vec[load_cnt];
                else
                    in_addr = addr_p11_vec[load_cnt-LANES];

                next_state = S_LOAD_BOTTOM_DATA;
            end

            S_LOAD_BOTTOM_DATA: begin
                if (load_cnt == 2*LANES-1)
                    next_state = S_COMPUTE;
                else
                    next_state = S_LOAD_BOTTOM_ADDR;
            end

            // ------------------------
            // COMPU
            // ------------------------
            S_COMPUTE: begin
                simd_valid_in = 1'b1;
                if (simd_valid_out)
                    next_state = S_WRITE;
            end

            S_WRITE: begin
                if (lane_wr_idx == LANES-1)
                    next_state = S_NEXT;
                else
                    next_state = S_WRITE;
            end

            S_NEXT: begin
                if (x_block == BLOCKS_X-1) begin
                    if (y_block == IMG_H-2)
                        next_state = S_DONE;
                    else
                        next_state = S_LOAD_TOP_ADDR;
                end
                else begin
                    next_state = S_LOAD_TOP_ADDR;
                end
            end

            S_DONE: begin
                done       = 1'b1;
                next_state = S_IDLE;
            end

            default: next_state = S_IDLE;
        endcase
    end

    // ===============================================================
    // 7. Máquina de estados: registro de estado y registros asociados
    //     *** AQUÍ ES DONDE SE INTEGRA allow_tick ***
    // ===============================================================

    integer i;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state       <= S_IDLE;
            x_block     <= '0;
            y_block     <= '0;
            load_cnt    <= '0;
            lane_wr_idx <= '0;

            for (i = 0; i < LANES; i = i + 1) begin
                p00_vec[i] <= '0;
                p10_vec[i] <= '0;
                p01_vec[i] <= '0;
                p11_vec[i] <= '0;
            end

        end else if (allow_tick) begin
            state <= next_state;

            case (state)
                S_IDLE: begin
                    if (start) begin
                        x_block     <= '0;
                        y_block     <= '0;
                        load_cnt    <= '0;
                        lane_wr_idx <= '0;
                    end
                end

                S_LOAD_TOP_ADDR: begin
                    // nada que registrar aquí
                end

                S_LOAD_TOP_DATA: begin
                    // Guardar p00/p10 según load_cnt
                    if (load_cnt < LANES)
                        p00_vec[load_cnt] <= in_rdata;
                    else
                        p10_vec[load_cnt-LANES] <= in_rdata;

                    load_cnt <= load_cnt + 1;
                end

                S_LOAD_BOTTOM_ADDR: begin
                    // nada que registrar aquí
                end

                S_LOAD_BOTTOM_DATA: begin
                    if (load_cnt < LANES)
                        p01_vec[load_cnt] <= in_rdata;
                    else
                        p11_vec[load_cnt-LANES] <= in_rdata;

                    load_cnt <= load_cnt + 1;
                end

                S_COMPUTE: begin
                    // nada adicional, SIMD se encarga internamente
                    load_cnt <= '0;
                end

                S_WRITE: begin
                    if (lane_wr_idx == LANES-1)
                        lane_wr_idx <= '0;
                    else
                        lane_wr_idx <= lane_wr_idx + 1;
                end

                S_NEXT: begin
                    if (x_block == BLOCKS_X-1) begin
                        x_block <= '0;
                        if (y_block == IMG_H-2)
                            y_block <= '0;
                        else
                            y_block <= y_block + 1;
                    end
                    else begin
                        x_block <= x_block + 1;
                    end

                    load_cnt    <= '0;
                    lane_wr_idx <= '0;
                end

                S_DONE: begin
                    // nada extra, done se maneja en always_comb
                end

                default: begin
                    state <= S_IDLE;
                end
            endcase
        end
    end

endmodule


